  ...
  api.get('/', cacheResponse('1 minute'),
    validate({
      query: {
        ...
        city: Joi.alternatives().try(Joi.string(), Joi.any().valid(null)).default(null, 'city'),
      },
    }),
    (req, res, next) => {
      ...
      archive(config, db, logger).maxstate(req.query.start, req.query.end, req.query.city)
